...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def _initialize_bezier_adapter(self):
        """Initialize BezierAdapter modules for the pipeline."""
        device = self._execution_device
        
        # Initialize BezierParameterProcessor
        self.bezier_processor = BezierParameterProcessor(
            output_resolution=(64, 64),  # Match latent resolution
            hidden_dim=128,
            max_curves=16,
            max_points_per_curve=8
        ).to(device)
        
        # Initialize other BezierAdapter modules
        self.condition_adapter = ConditionInjectionAdapter(
            clip_dim=768,
            t5_dim=4096,
            hidden_dim=1536,
            output_dim=3072,  # Match FluxTransformer inner_dim
            num_heads=8,
            lora_rank=64
        ).to(device)
        ...
        self.density_sampler = DensityAdaptiveSampler()
        ...
        print("BezierAdapter modules initialized successfully")

    def _prepare_bezier_tensor(self, bezier_curves):
        """Convert bezier curves to tensor format for processing."""
        from .bezier_adapter.data_utils import batch_bezier_curves
        
        # Handle different input formats
        if isinstance(bezier_curves, str):
            # Path to bezier JSON file
            from .bezier_adapter.data_utils import load_bezier_json, convert_json_to_bezier_curves
            bezier_data = load_bezier_json(bezier_curves)
            curves = convert_json_to_bezier_curves(bezier_data)
        elif isinstance(bezier_curves, list):
            if len(bezier_curves) > 0 and isinstance(bezier_curves[0], BezierCurve):
                ...
                for curve_points in bezier_curves:
                    if len(curve_points) >= 4:
                        control_points = torch.tensor(curve_points[:4], dtype=torch.float32)
                        curve = BezierCurve(control_points=control_points)
                        curves.append(curve)
        else:
            raise ValueError(f"Unsupported bezier_curves format: {type(bezier_curves)}")
        
        # Convert to batched tensor format
        config = BezierConfig(max_curves=16, max_points_per_curve=8)
        bezier_points, bezier_mask = batch_bezier_curves(curves, config)
        
        # Move to device
        device = self._execution_device
        bezier_points = bezier_points.to(device)
        bezier_mask = bezier_mask.to(device) if bezier_mask is not None else None
    ...
    def __call__(
        ...
        # Process Bézier curves if provided
        if bezier_curves is not None:
            # Initialize BezierAdapter modules if not already done
            if not hasattr(self, 'bezier_processor'):
                self._initialize_bezier_adapter()
            
            # Convert bezier curves to tensor format
            bezier_tensor = self._prepare_bezier_tensor(bezier_curves)
            
            # Generate density maps from Bézier curves
            density_maps, field_maps = self.bezier_processor(bezier_tensor)
            
            # Store in joint_attention_kwargs for transformer
            if self._joint_attention_kwargs is None:
                self._joint_attention_kwargs = {}
            self._joint_attention_kwargs.update({
                'bezier_density': density_maps,
                'bezier_fields': field_maps
            })
        else:
            density_maps = None
            field_maps = None
